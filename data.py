...
#     dnames = ['brown']
    ...
    print('Using N size = ',n_size)
    for dname in dnames:
        print("-----------------------")
        print(dname)
        data = read_texts("data/corpora.tar.gz", dname)
        datas.append(data)
        new_test = data.train[:1000]
        new_train = data.train[1000:]
        class Data: pass
        new_data = Data()
        new_data.train = new_train
        new_data.test =new_test
#         print('last sentence: ',data.train[-1])
#         model = learn_unigram(data)
#         print(data.train[0])
        n_model = learn_Ngram(new_data,n_size)
#         models.append(model)
        print('model len: ',len(n_model.phraseFrequency))
        ngram_models.append(n_model)
...
#             perp_dev[i][j] = ngram_models[i].perplexity(datas[j].dev)
            perp_test[i][j] = ngram_models[i].perplexity(datas[j].test)
            perp_train[i][j] = ngram_models[i].perplexity(datas[j].train)
...
#     print_table(perp_dev, dnames, dnames, os.path.join(output_dir, "table-dev.tex"))
    ...
    save_lms(dnames, ngram_models, output_dir)
